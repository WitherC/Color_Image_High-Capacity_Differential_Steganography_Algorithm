...
def g_loss_fn(u_net_s,extractor,Xunet,SRnet,ZhuNet, batch_x, batch_y, is_training, batch_size = 16):
    ...
    Zhu_array = np.zeros([batch_size, 2], dtype=np.float32)
    for i in range(0, batch_size):
        Zhu_array[i, 0] = 1
    Img_label_Zhu = tf.constant(Zhu_array)
    d_logits_Zhu = ZhuNet(stego / 255.0, training=False)
    correct_prediction_Zhu = tf.equal(tf.argmax(d_logits_Zhu, 1), tf.argmax(Img_label_Zhu, 1))
    loss_Zhu = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_logits_Zhu, labels=Img_label_Zhu))
    ...
    Antiloss = loss_SR + loss_XU + loss_Zhu
    Gloss_All = 0.05*Antiloss + (Gloss_MSE_extra+Gloss_MSE_stego) + (Gloss_ssim_extra+Gloss_ssim_stego)

    return Gloss_All,Sloss,Eloss,Antiloss,Gloss_MSE_stego,Gloss_MSE_extra,Gloss_ssim_stego,Gloss_ssim_extra,stego,extra,loss_SR,loss_XU,loss_Zhu
...
def Zhu_loss_fn(u_net_s, ZhuNet, batch_x, batch_y, is_training, batch_size=16):
    batch_x_r, batch_x_g, batch_x_b = tf.split(batch_x, num_or_size_splits=3, axis=-1)
    batch_x_r = tf.cast(batch_x_r, dtype=tf.float32)
    batch_x_g = tf.cast(batch_x_g, dtype=tf.float32)
    ...
    batch_Zhu2 = tf.concat([batch_x, stego], 0)
    Zhu_array2 = np.zeros([batch_size * 2, 2], dtype=np.float32)
    for i in range(0, batch_size):
        Zhu_array2[i, 0] = 1
    for i in range(batch_size, batch_size * 2):
        Zhu_array2[i, 1] = 1
    Img_label_Zhu2 = tf.constant(Zhu_array2)
    d_logits_Zhu2 = ZhuNet(batch_Zhu2 / 255.0, is_training)
    correct_prediction_Zhu2 = tf.equal(tf.argmax(d_logits_Zhu2, 1), tf.argmax(Img_label_Zhu2, 1))
    accuracyD_Zhu2 = tf.reduce_mean(tf.cast(correct_prediction_Zhu2, tf.float32))

    loss_Zhu2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_logits_Zhu2, labels=Img_label_Zhu2))

    loss_all_Zhu = loss_Zhu2
    return loss_all_Zhu,loss_Zhu2, accuracyD_Zhu2
...
def main():
    ...
    path1 = r"C:\Users\admin\Desktop\COCO_trainA"
    path2 = r"C:\Users\admin\Desktop\COCO_trainB"
...
#-----------Adam优化时
        ...
        Zhu_optimizer = tf.optimizers.Adam(learning_rate=0.00001, beta_1=0.5)
...
#---------------------
        with tf.GradientTape() as tape:
            loss_all_Zhu,loss_Zhu2, accuracyD_Zhu2 = SR_loss_fn(u_net_s, ZhuNet, batch_x, batch_y, is_training)
        grads = tape.gradient(loss_all_Zhu, ZhuNet.trainable_variables)
        Zhu_optimizer.apply_gradients(zip(grads, ZhuNet.trainable_variables))
        ...
        with tf.GradientTape() as tape:
            Gloss_All,Sloss,Eloss,Antiloss,Gloss_MSE_stego,Gloss_MSE_extra,Gloss_ssim_stego,Gloss_ssim_extra,stego,extra,loss_SR,loss_XU,loss_Zhu = g_loss_fn(u_net_s,extractor,Xunet,SRnet,ZhuNet, batch_x, batch_y, is_training)
...
#------------------
        ...
        print('---------------------------------------------------------------------------------------------------------','\n',
              ...
              'gloss_Zhu', float(loss_Zhu), '\n',
              'Time:',float(time_end)-float(time_start),'\n',
              '---------------------------------------------------------------------------------------------------------')

        if epoch % 10 == 0:
            imageio.imwrite(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\stego" + '/' + str(epoch) + '.png', np.uint8(stego[0, :, :, :]))
            imageio.imwrite(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\stego" + '/' + str(epoch) + 'ori' + '.png',np.uint8(batch_x[0, :, :, :]))
            imageio.imwrite(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\extra" + '/' + str(epoch) + '.png', np.uint8(extra[0, :, :, :]))
            imageio.imwrite(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\extra" + '/' + str(epoch) + 'ori' + '.png',np.uint8(batch_y[0, :, :, :]))
        ...
        if epoch % 1000 == 0:
             u_net_s.save_weights(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\weight" + '/' + 'u_net_s' + '%d' % (epoch) + '.ckpt')
             extractor.save_weights(r"D:\WHC\img_img_test\2024.5.9 XuSRZhu\weight" + '/' + 'extractor' + '%d' % (epoch) + '.ckpt')
