...
def g_loss_fn(u_net_s,extractor,Xunet,SRnet, batch_x, batch_y, is_training, batch_size = 16):
    ...
    batch_x_b = tf.cast(batch_x_b, dtype=tf.float32)
    ...
    extra_psnr = tf.reduce_mean(tf.image.psnr(tf.cast(d, dtype='uint8'), tf.cast(c, dtype='uint8'), max_val=255.0))
    extra_ssim = tf.reduce_mean(tf.image.ssim(tf.cast(d, dtype='uint8'), tf.cast(c, dtype='uint8'), max_val=255.0))
    ...
    return Gloss_All,extra_psnr,extra_ssim, Sloss,Eloss,Antiloss,Gloss_MSE_stego,Gloss_MSE_extra,Gloss_ssim_stego,Gloss_ssim_extra,stego,extra,loss_SR,loss_XU
...
def Xu_loss_fn(u_net_s, Xunet, batch_x, batch_y, is_training, batch_size=16):
    ...
    loss_XU2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_logits_XU2, labels=Img_label_XU2))
...
def main():
    ...
    csv_extra = '1000extra.csv'
    with open(csv_extra, mode='w', newline='') as file:
        writer = csv.writer(file)
        # 写入CSV文件的表头
        writer.writerow(['iteration', 'psnr', 'ssim'])
    ...
    u_net_s.load_weights(r"D:\WHC\img_img_test\weightu_net_s20000.ckpt").expect_partial()
    extractor.load_weights(r"D:\WHC\img_img_test\weightextractor20000.ckpt").expect_partial()
    ...
    # log_dir_Gloss = r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\log" + current_time
    # log_dir_MSE_stego = r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\log" + current_time
    # log_dir_MSE_extra = r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\log" + current_time
    # log_dir_Loss_XU = r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\log" + current_time
    # log_dir_Loss_SR = r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\log" + current_time
    #
    ...
    for epoch in range(epochs):
        ...
        count_jian = seed / 3
        count_jian = int(count_jian)
        lr1 = 0.001
        lr2 = 0.001
        # XU_optimizer = tf.optimizers.Adam(learning_rate=0.001, beta_1=0.5)
        #e_optimizer = tf.optimizers.Adam(learning_rate=0.0005)
...
#-----------Adam优化时
        ...
        # e_optimizer = tf.optimizers.Adam(learning_rate=0.000001,beta_1=0.5)
        # g_optimizer = tf.optimizers.Adam(learning_rate=0.000001,beta_1=0.5)
#--------------
#------------Lion优化时
        XU_optimizer = Lion(learning_rate=0.000001,wd=0.00035)
        SR_optimizer = Lion(learning_rate=0.000001,wd=0.00035)
        e_optimizer = Lion(learning_rate=0.0000001,wd=0.003)
        g_optimizer = Lion(learning_rate=0,wd=0.003)
...
#---------------------
        ...
        with tf.GradientTape() as tape:
            Gloss_All,extra_psnr,extra_ssim,Sloss,Eloss,Antiloss,Gloss_MSE_stego,Gloss_MSE_extra,Gloss_ssim_stego,Gloss_ssim_extra,stego,extra,loss_SR,loss_XU = g_loss_fn(u_net_s,extractor,Xunet,SRnet, batch_x, batch_y, is_training)
        ...
        # with tf.GradientTape() as tape:
        #     Gloss_All,extra_psnr,Sloss,Eloss,Antiloss,Gloss_MSE_stego,Gloss_MSE_extra,Gloss_ssim_stego,Gloss_ssim_extra,stego,extra,loss_SR,loss_XU = g_loss_fn(u_net_s,extractor,Xunet,SRnet, batch_x, batch_y, is_training)
...
#----------------
        ...
        #     writer = csv.writer(file)
        #     writer.writerow([int(epoch), int(epochs) + 1, loss_XU, float(time_end)-float(time_start)])
        # with open(csv_SR, mode='a', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow([int(epoch), int(epochs) + 1, loss_SR, float(time_end)-float(time_start)])
#------------------
        with open(csv_extra, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([int(epoch), float(extra_psnr), float(extra_ssim)])
        print('---------------------------------------------------------------------------------------------------------','\n',
              'epoch：',seed,'训练次数：',epoch,'\n',
              ...
              'Time:',float(time_end)-float(time_start), "extra_psnr:",float(extra_psnr),'\n',
              '---------------------------------------------------------------------------------------------------------')

        # if epoch % 10 == 0:
        #     imageio.imwrite(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\stego" + '/' + str(epoch) + '.png', np.uint8(stego[0, :, :, :]))
        #     imageio.imwrite(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\stego" + '/' + str(epoch) + 'ori' + '.png',np.uint8(batch_x[0, :, :, :]))
        #     imageio.imwrite(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\extra" + '/' + str(epoch) + '.png', np.uint8(extra[0, :, :, :]))
        #     imageio.imwrite(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\extra" + '/' + str(epoch) + 'ori' + '.png',np.uint8(batch_y[0, :, :, :]))
        ...
        # if epoch % 500 == 0:
        #     u_net_s.save_weights(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\weight" + '/' + 'u_net_s' + '%d' % (epoch) + '.ckpt')
        #     extractor.save_weights(r"D:\WHC\img_img_test\2023.12.18\lr\zai xun lian\0.0001\weight" + '/' + 'extractor' + '%d' % (epoch) + '.ckpt')
